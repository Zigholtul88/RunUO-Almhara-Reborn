...
namespace Server.Engines.Quests.Collector
{
	...
	{
		[Constructable]
		public ElwoodMcCarrin() : base( "(Collector)" )
		{
		}
		...
		{
			Name = "Elwood McCarrin the well-known collector";
			Body = 0x190;
			Female = false;
			Hue = 0x83ED;
		...
		public bool GiveReward( Mobile to )
		{
			...
			bag.DropItem( new SkillSlotDeedQuestReward() );
			bag.DropItem( new WeightIncreaseDeed() );
			bag.DropItem( new QuestGoodieBag() );
			...
			if ( Utility.RandomBool() )
			{
				...
				if ( Core.AOS )
				{
					BaseRunicTool.ApplyAttributesTo( weapon, 5, 20, 30 );
				}
				else
				{
					weapon.DamageLevel = (WeaponDamageLevel)BaseCreature.RandomMinMaxScaled( 2, 5 );
					weapon.AccuracyLevel = (WeaponAccuracyLevel)BaseCreature.RandomMinMaxScaled( 2, 5 );
					weapon.DurabilityLevel = (WeaponDurabilityLevel)BaseCreature.RandomMinMaxScaled( 2, 5 );
				}
			...
			else
			{
				...
				if ( Core.AOS )
				{
					...
					if ( item is BaseArmor )
						BaseRunicTool.ApplyAttributesTo( (BaseArmor)item, 5, 20, 30 );
					else if ( item is BaseJewel )
						BaseRunicTool.ApplyAttributesTo( (BaseJewel)item, 5, 20, 30 );
				}
				else
				{
					...
					armor.ProtectionLevel = (ArmorProtectionLevel)BaseCreature.RandomMinMaxScaled( 2, 5 );
					armor.Durability = (ArmorDurabilityLevel)BaseCreature.RandomMinMaxScaled( 2, 5 );
				}
