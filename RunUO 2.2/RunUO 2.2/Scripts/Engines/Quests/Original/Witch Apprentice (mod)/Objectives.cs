...
namespace Server.Engines.Quests.Hag
{
	public class FindApprenticeObjective : QuestObjective
	{
		private static Point3D[] m_CorpseLocations = new Point3D[]
			{
				new Point3D( 1265, 923, 0 ),
				new Point3D( 1278, 900, 0 ),
				new Point3D( 1242, 948, 0 ),
				new Point3D( 1249, 904, 0 ),
				new Point3D( 1255, 931, 0 )
			};
		...
		public override object Message
		{
			get
			{
				return "Buried within Zaythalor Forest is the graveyard located north of Hammerhill. After reaching your destination, be on the lookout for any sign of the Hag's apprentice.";
		...
		public override void CheckProgress()
		{
			...
			if ( ( m_Corpse == null || m_Corpse.Deleted ) && ( map == Map.Malas ) && player.InRange( m_CorpseLocation, 8 ) )
			{
	...
	public class KillImpsObjective : QuestObjective
	{
		...
		public override void OnComplete()
		{
			...
			mapItem.SetDisplay( loc.X - 300, loc.Y - 300, loc.X + 300, loc.Y + 300, 300, 300 );
