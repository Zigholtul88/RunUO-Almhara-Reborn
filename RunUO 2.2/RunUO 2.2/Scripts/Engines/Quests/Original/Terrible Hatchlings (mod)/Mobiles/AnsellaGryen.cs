...
namespace Server.Engines.Quests.Zento
{
	public class AnsellaGryen : BaseQuester
	{
		...
		public override void OnTalk( PlayerMobile player, bool contextMenu )
		{
			...
			{
				if ( qs.IsObjectiveInProgress( typeof( KillObjective ) ) )
				{
				...
				else
				{
					...
					if ( obj != null && !obj.Completed )
					{
						...
						cont.DropItem( new Gold( Utility.RandomMinMax( 182, 204 ) ) );
						cont.DropItem( new QuestGoodieBag() );
						cont.DropItem( new SkillSlotDeedQuestReward() );

						if ( Utility.RandomBool() )
						{
							BaseWeapon weapon = Loot.RandomWeapon( true );
							BaseRunicTool.ApplyAttributesTo( weapon, 4, 25, 30 );
						...
						else
						{
							...
							BaseRunicTool.ApplyAttributesTo( armor, 4, 25, 30 );
