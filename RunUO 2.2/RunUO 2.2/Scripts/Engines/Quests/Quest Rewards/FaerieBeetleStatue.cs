...
{
	public class FaerieBeetleStatue : Item
	{
		public override string DefaultName
		{
			get { return "A Faerie Beetle Statue"; }
		}

		[Constructable]
		public FaerieBeetleStatue() : base( 9743 )
		{
                        Movable = true;
			Weight = 5.0;
                        Hue = 181;
		...
		public FaerieBeetleStatue( Serial serial ) : base( serial )
		{
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			{
				from.CloseGump( typeof( FaerieBeetleStatueWarningGump ) );
				from.SendGump( new FaerieBeetleStatueWarningGump( from ) );
			}
			else
			{
	...
	public class FaerieBeetleStatueWarningGump : Gump
	{
		public FaerieBeetleStatueWarningGump( Mobile m): base( 0, 0 )
		{
			this.Closable=false;
			...
			this.AddLabel(282, 151, 1153, @"Summon Faerie Beetle!");
			this.AddButton(226, 469, 247, 248, 0, GumpButtonType.Reply, 0);
			this.AddButton(351, 469, 241, 242,  1, GumpButtonType.Reply, 0);
			this.AddHtml( 157, 181, 345, 279, "This statue when used will summon a faerie beetle collector.<br><br><i>Its much weaker than a standard faerie beetle and will take up 1 control slot, though it could be useful as a mild distraction.</i><br><br>Are you sure you want to commit?", false, true);
		}
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			switch( info.ButtonID )
			{
				case 0:
				{
			            Item a = from.Backpack.FindItemByType( typeof(FaerieBeetleStatue) );
			            if ( a != null )
				        ...
				        a.Delete();
           		                ...
           		                from.FixedParticles( 0x373A, 10, 15, 5036, EffectLayer.Head ); 
               		                from.PlaySound( 521 );
        		                FaerieBeetleCollector faeriebeetle = new FaerieBeetleCollector();
        		                faeriebeetle.Controlled = true;
        		                faeriebeetle.ControlMaster = from;
        		                faeriebeetle.IsBonded = true;
					faeriebeetle.MoveToWorld( from.Location, from.Map );
               		                from.SendMessage( "You summon a faerie beetle collector." );
					from.CloseGump( typeof( FaerieBeetleStatueWarningGump ) );
