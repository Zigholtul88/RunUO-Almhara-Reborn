...
namespace Server.Engines.Craft
{
	...
	public class CraftItem
	{
		...
		public bool NeedCauldron
		{
			get { return m_NeedCauldron; }
			set { m_NeedCauldron = value; }
		}
		public bool NeedDistillery
		{
			get { return m_NeedDistillery; }
			set { m_NeedDistillery = value; }
		}
		public bool NeedSteamPoweredBeverageMaker
		{
			get { return m_NeedSteamPoweredBeverageMaker; }
			set { m_NeedSteamPoweredBeverageMaker = value; }
		}
		...
		private static readonly int[] m_Mills = new int[]
			{
                ...
                private static readonly int[] m_Cauldrons = new int[]
		        {
			        0x0974, 0x0975
		        };

                private static readonly int[] m_Distillerys = new int[]
		        {
			        0x2DDB, 0x2DDC
		        };

                private static readonly int[] m_SteamPoweredBeverageMakers = new int[]
		        {
			        0x2D4A, 0x2D4A
		        };
		...
		public bool ConsumeRes( Mobile from, Type typeRes, CraftSystem craftSystem, ref int resHue, ref int maxAmount, ConsumeType consumeType, ref object message, bool isFailure )
		{
			...
			{
                                from.SendMessage( "You must be near a cauldron to do that." );
				return false;
                        }

                        if ( m_NeedDistillery && !Find( from, m_Distillerys ) )
                        {
                               from.SendMessage( "You must be near a distillery to do that." );
                               return false;
                        }

                        if ( m_NeedSteamPoweredBeverageMaker && !Find( from, m_SteamPoweredBeverageMakers ) )
                        {
                               from.SendMessage( "You must be near a Steam Powered Beverage Maker to do that." );
                               return false;
                        }
