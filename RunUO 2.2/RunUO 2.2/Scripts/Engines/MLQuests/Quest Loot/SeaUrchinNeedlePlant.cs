...
{ 
	public class SeaUrchinNeedlePlant : Item
	{ 
		...
		[Constructable]
		public SeaUrchinNeedlePlant() : base( Utility.RandomList( 14507, 14508 ) ) 
		{
			Name = "sea urchin plant";
		...
		public SeaUrchinNeedlePlant( Serial serial ) : base( serial ) 
		{ 
		} 
		...
		{ 
			if ( from.InRange( this.GetWorldLocation(), 2 ) ) 
		        { 
			        ...
			        SeaUrchinNeedles crop = new SeaUrchinNeedles( pick ); 
				from.AddToBackpack( crop );
			        from.SendMessage( "You harvest some sea urchin needles." ); 
				this.Delete();
			}
		        else
		        { 
