		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				else if ( targeted is BaseCreature )
				{
					...
					if ( !c.IsDeadPet )
					{
						if ( c.Body.IsAnimal || c.Body.IsMonster || c.Body.IsSea || c is RunningPants )
						{
		...
		public AnimalLoreGump( BaseCreature c ) : base( 250, 50 )
		{
			...
			if ( Core.AOS )
			{
				int y = 276;
                  ...
                  {
                      AddHtml( 153, y, 160, 18, "<BASEFONT COLOR=#4A3929>Min Taming Level</BASEFONT>", false, false ); // Min Taming Level
                      AddHtml( 320, y, 35, 18, FormatDouble( c.MinTameSkill ), false, false );
                      y += 18;
                  }
				if ( Core.SE )
				{
