	...
	class SpiritSpeak
	{
		...
		private class SpiritSpeakSpell : Spell
		{
			...
			public override void OnCast()
			{
				...
				int max, min, mana, number, min_mana;

				/* 
				-- Changed behavior. Now this is based completely on Necromancy skill.  The mininmum
				-- heal amount = 1/4th of necro skill, max =(min + 1/10th of Necro skill.  The cost in mana
				-- is 1/4th of Necro skill, with a minimum mana cost of 1/10th of Necro skill.
				-- The end result is that unless you have some necro skill, you will not be healing at all.
				-- If there is a corpse nearby, the cost in mana is half.
				-- If you do not have the necromancy skill, you do not see any messages regarding healing
				*/
				if ( toChannel != null )
				{
					min = (int)(Caster.Skills[SkillName.Necromancy].Value * 0.25);
					max = min + (int)(Caster.Skills[SkillName.Necromancy].Value * 0.1);
					mana = (int)(Caster.Skills[SkillName.Necromancy].Value * 0.25)/2;
					min_mana = (int)(Caster.Skills[SkillName.Necromancy].Value * 0.05);
					if ( mana < min_mana)
						mana = min_mana;
				...
				else
				{
					min = (int)(Caster.Skills[SkillName.Necromancy].Value * 0.25);
					max = min + (int)(Caster.Skills[SkillName.Necromancy].Value * 0.1);
					mana = (int)(Caster.Skills[SkillName.Necromancy].Value * 0.25);
					min_mana = (int)(Caster.Skills[SkillName.Necromancy].Value * 0.1);
					if ( mana < min_mana)
						mana = min_mana;
					number = 1061286; // You channel your own spiritual energy to heal your wounds.
				}

				if ( (Caster.Mana < mana) && (Caster.Skills[SkillName.Necromancy].Value > 0 ))
				{
					Caster.SendLocalizedMessage( 1061285 ); // You lack the mana required to use this skill.
				}
				else if (Caster.Skills[SkillName.Necromancy].Base > 0 )
				{
					Caster.CheckSkill( SkillName.Necromancy, 0.0, 500.0 );

					if ( Utility.RandomDouble() > (Caster.Skills[SkillName.Necromancy].Value / 100.0) )
					{
						Caster.SendMessage( "You failed to heal with channeled energy." );
					}
					else
					{
						if ( toChannel != null )
