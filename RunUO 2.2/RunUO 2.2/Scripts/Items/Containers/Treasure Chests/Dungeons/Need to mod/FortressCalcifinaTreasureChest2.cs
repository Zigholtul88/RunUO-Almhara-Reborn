...
namespace Server.Items
{
    ...
    public class FortressCalcifinaTreasureChest2 : LockableContainer
    {
        ...
        [Constructable]
        public FortressCalcifinaTreasureChest2() : base( 0xE43 )
        {
...
/////////////////////////////////////// Rare Items
		...
		DropItem( new DyeTub() );
 		...
 		if ( Utility.RandomDouble() < 0.15 )
			...
			switch ( Utility.Random( 38 ) )
			{
				...
				default: weapon = new Dagger(); break;
			        ...
			        BaseRunicTool.ApplyAttributesTo( weapon, 4, 22, 25 );
 		...
 		if ( Utility.RandomDouble() < 0.15 )
			...
			switch ( Utility.Random( 13 ) )
			{
				...
				default: armor = new StuddedChest(); break;
			        ...
			        BaseRunicTool.ApplyAttributesTo( armor, 4, 22, 25 );
 		...
 		if ( Utility.RandomDouble() < 0.15 )
			...
			switch ( Utility.Random( 5 ) )
			{
				...
				BaseRunicTool.ApplyAttributesTo( hat, 4, 22, 25 );
 		...
 		if ( Utility.RandomDouble() < 0.15 )
            {
			  ...
			  if ( Core.AOS )
		          BaseRunicTool.ApplyAttributesTo( shield, 4, 22, 25 );              
 		...
 		if ( Utility.RandomDouble() < 0.15 )
            {
			  ...
			  if ( Core.AOS )
		          BaseRunicTool.ApplyAttributesTo( bracelet, 4, 22, 25 );              
 		...
 		if ( Utility.RandomDouble() < 0.15 )
            {
			  ...
			  if ( Core.AOS )
		          BaseRunicTool.ApplyAttributesTo( earrings, 4, 22, 25 );              
 		...
 		if ( Utility.RandomDouble() < 0.15 )
            {
			  ...
			  if ( Core.AOS )
		          BaseRunicTool.ApplyAttributesTo( necklace, 4, 22, 25 );              
 		...
 		if ( Utility.RandomDouble() < 0.15 )
            {
			  ...
			  if ( Core.AOS )
		          BaseRunicTool.ApplyAttributesTo( ring, 4, 22, 25 );              
        ...
        public FortressCalcifinaTreasureChest2( Serial serial ) : base( serial )
        {
        }
