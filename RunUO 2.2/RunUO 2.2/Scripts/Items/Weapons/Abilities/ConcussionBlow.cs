...
{
	/// <summary>
	/// This devastating strike is most effective against those who are in good health and whose reserves of stamina are low, or vice versa.
	/// </summary>
	public class ConcussionBlow : WeaponAbility
	{
		...
		public override bool OnBeforeDamage( Mobile attacker, Mobile defender )
		{
			...
			if ( defender.HitsMax > 0 ) 
			{
				...
				double stamPercent = 0;

				if ( defender.StamMax > 0 )
					stamPercent = ( (double)defender.Stam / (double)defender.StamMax ) * 100.0;

				damage += Math.Min( (int)(Math.Abs( hitsPercent - stamPercent ) / 4), 20 );
			}
