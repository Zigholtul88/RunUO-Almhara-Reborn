...
namespace Server.Items
{
	...
	public abstract class BaseWeapon : Item, IWeapon, IFactionItem, ICraftable, ISlayer, IDurability
	{
		...
		public virtual double ScaleDamageAOS( Mobile attacker, double damage, bool checkSkills )
		{
			if ( checkSkills )
			{
				...
				if ( this is ShepherdsCrook )
					attacker.CheckSkill( SkillName.AnimalLore, 0.0, 100.0 ); // Passively check Animal Lore for gain
			}
			...
			double   animalloreBonus = GetBonus( attacker.Skills[SkillName.AnimalLore].Value,	0.200, 100.0, 10.00 );
			...
			// Horrific Beast transformation gives a +50% bonus to damage.
			if( TransformationSpellHelper.UnderTransformation( attacker, typeof( HorrificBeastSpell ) ) )
				damageBonus += 50;

			// Divine Fury gives a +25% bonus to damage.
			if ( Spells.Chivalry.DivineFurySpell.UnderEffect( attacker ) )
				damageBonus += 25;
			...
			double totalBonus = strengthBonus + anatomyBonus + tacticsBonus + lumberBonus + animalloreBonus + ((double)(GetDamageBonus() + damageBonus) / 100.0);
		...
		public virtual double ScaleDamageOld( Mobile attacker, double damage, bool checkSkills )
		{
			...
			/* Compute animallore bonus
			 * : 1% bonus for every 5 points of animallore
			 * : +10% bonus at Grandmaster or higher
			 */
			double animalloreBonus;

			if ( this is ShepherdsCrook )
			{
				double animalloreValue = attacker.Skills[SkillName.AnimalLore].Value;

				animalloreBonus = (animalloreValue / 5.0) / 100.0;

				if ( animalloreValue >= 100.0 )
					animalloreBonus += 0.1;
			}
			else
			{
			...
			// Apply bonuses
			damage += (damage * tacticsBonus) + (damage * strBonus) + (damage * anatomyBonus) + (damage * lumberBonus) + (damage * animalloreBonus) + (damage * qualityBonus) + ((damage * VirtualDamageBonus) / 100);
