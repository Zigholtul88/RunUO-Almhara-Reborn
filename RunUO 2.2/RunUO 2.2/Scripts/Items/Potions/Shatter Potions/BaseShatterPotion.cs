...
{
	public abstract class BaseShatterPotion : BasePotion
	{
		...
		public BaseShatterPotion( Serial serial ) : base( serial )
		{
		}
		...
		{
			private BaseShatterPotion m_Potion;

			public BaseShatterPotion Potion
			{
				get{ return m_Potion; }
			}

			public ThrowTarget( BaseShatterPotion potion ) : base( 12, true, TargetFlags.None )
			{
				m_Potion = potion;
			}
		...
		public void Explode( Mobile from, bool direct, Point3D loc, Map map )
		{
			...
			Effects.PlaySound(loc, map, 0x390);
			Effects.PlaySound(loc, map, 0x38D);

			Effects.SendLocationEffect(loc, map, 0x36B0, 9, 10, 1152, 0);
			Effects.SendLocationEffect(loc, map, 0x480, 1, 10, 1152, 1);
			int alchemyBonus = 0;
			...
			foreach ( object o in eable )
			{
				...
				else if ( o is BaseShatterPotion && o != this )
				{
					toExplode.Add( o );
				}
			}
			...
			for ( int i = 0; i < toExplode.Count; ++i )
			{
				...
				if ( o is Mobile )
				{
					...
					AOS.Damage( m, from, damage, 0, 0, 100, 0, 0 );
				}
				else if ( o is BaseShatterPotion )
				{
					BaseShatterPotion pot = (BaseShatterPotion)o;
