...
{
	public class CrackedStamRegenGem : Item
	{
		[Constructable]
		public CrackedStamRegenGem() : base( 0x3198 )
		{
			Name = "Cracked Stam Regen Gem";
			Weight = 1.0;
			Hue = 1372;
		}

		public CrackedStamRegenGem( Serial serial ) : base( serial )
		{
		}

		private string m_name ="Cracked Stam Regen Gem";
		...
		{
			private CrackedStamRegenGem m_deed;

			public InternalTarget( CrackedStamRegenGem deed ) : base( 3, false, TargetFlags.None )
			{
				m_deed = deed;
			}
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				if ( targeted is BaseArmor || targeted is BaseJewel || targeted is BaseClothing )
				{
					...
					if ( a !=null )
					{
						if ( a.Attributes.RegenStam!=0 )
						{
							a.Attributes.RegenStam=0;
							from.SendMessage("Gem absorbed items glow, and disintegrated with bright flash");
						}
                                                else
						{
							a.Attributes.RegenStam=1;
							from.SendMessage("Glow from gem was transfered to item, gem wanished into thin air");
						}
					}
                                        else if ( j!=null )
					{
						if ( j.Attributes.RegenStam!=0 )
						{
							j.Attributes.RegenStam=0;
                                                        from.SendMessage("Gem absorbed items glow, and disintegrated with bright flash");
						}
                                                else
						{
							j.Attributes.RegenStam=1;
                                                        from.SendMessage("Glow from gem was transfered to item, gem wanished into thin air");
					        }
					} 
                                        if ( c!=null )
					{
						if ( c.Attributes.RegenStam!=0 )
						{
							c.Attributes.RegenStam=0;
                                                        from.SendMessage("Gem absorbed items glow, and disintegrated with bright flash");
						}
                                                else
						{
							c.Attributes.RegenStam=1;
                                                        from.SendMessage("Glow from gem was transfered to item, gem wanished into thin air");
						}
					}
