...
namespace Server.Multis
{
	...
	public class HousePlacement
	{
		...
		public static HousePlacementResult Check( Mobile from, int multiID, Point3D center, out ArrayList toMove )
		{
			...
			if ( map == Map.Ilshenar || map == Map.Felucca || map == Map.Malas || map == Map.Tokuno || from.AccessLevel >= AccessLevel.GameMaster || SpellHelper.IsFeluccaT2A( map, center ) )
			...
			if ( map == Map.Malas && ( multiID == 0x007C || multiID == 0x007E ) )
				return HousePlacementResult.InvalidCastleKeep;
	                ...
	                if ( from.Region.Name == "Sunnyside Resort Housing" )
				return HousePlacementResult.Valid; // Players can place houses here
