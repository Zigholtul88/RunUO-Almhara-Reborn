...
{
      public class RangerClassGate : Item
      {
                [Constructable]
                public RangerClassGate() : base(0xF6C)
                {
	                Movable = false;
	                Light = LightType.Circle300;
	                Name = "Ranger Class Gate"; 
                }

                public RangerClassGate(Serial serial) : base(serial)
                {
                }
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			int version = reader.ReadInt();
                ...
                { 
                        if ( m.Female == false )
                        {
				m.CloseGump( typeof( RangerClassGump ) );
				m.SendGump( new RangerClassGump( m ) );
	...
	public class RangerClassGump : Gump
	{
		public RangerClassGump( Mobile m ): base( 0, 0 )
		{
			this.Closable=false;
			...
			this.AddLabel(282, 151, 1153, @"Way of the Ranger!");
			this.AddButton(226, 469, 247, 248, 0, GumpButtonType.Reply, 0);
			this.AddButton(351, 469, 241, 242,  1, GumpButtonType.Reply, 0);
			this.AddHtml( 157, 181, 345, 279, "Proceed through this portal if you seek on joining the ways of the Ranger.<br><br><i>Once you've made your selection, you will be drawn to the outside gates of this island and thus will be unable to return.</i><br><br>Are you sure you want to commit?", false, true);
		}
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			switch( info.ButtonID )
			{
				case 0:
				{
                                        ...
                                        from.Skills.Archery.Base = 50;
                                        from.Skills.Fletching.Base = 50;
                                        from.Skills.Healing.Base = 50;
                                        from.Skills.Tactics.Base = 25;

                                        from.BankBox.DropItem( new RangerSpellbook() );

                                        from.BankBox.DropItem( new RangerFamiliarScroll() );
                                        from.BankBox.DropItem( new RangerHuntersAimScroll() );

                                        from.AddToBackpack( new RangerSkillGuide() );
                                        from.BankBox.DropItem( new Arrow(500) );

                                        from.BankBox.DropItem( new ElvenQuiver() );
                                        from.BankBox.DropItem( new FletcherTools() );
		                        ...
		                        from.FixedEffect( 0x376A, 10, 16 );
                                        ...
                                        World.Broadcast( 0x35, true, "Another has chosen thy path of the Ranger!" );
					from.CloseGump( typeof( RangerClassGump ) );

					break;
				}
				case 1:
				{
					from.SendMessage( "You decide that the way of the Ranger is not in your best interests." );
					from.CloseGump( typeof( RangerClassGump ) );
