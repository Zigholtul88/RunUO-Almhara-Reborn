...
{
	[CorpseName( "a clockwork overseer's corpse" )]
	public class ExodusOverseer : BaseCreature
	{
		...
		{
			if( from != null && from.Alive && 0.05 > Utility.RandomDouble() )
			{
				SendEBolt( from );
			}
			...
			else if ( m_FieldActive && !CanUseField )
			{
				m_FieldActive = false;
                                ...
                                from.SendMessage( "The barrier has been deactivated." ); 
		...
		public override void OnGotMeleeAttack( Mobile attacker )
		{
			...
			if( attacker != null && attacker.Alive && attacker.Weapon is BaseRanged && 0.05 > Utility.RandomDouble() )
			{
				SendEBolt( attacker );
			}
		}
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			if( this.Name == "Exodus Overseer" )
				this.Name = "a clockwork overseer";
		}
	}
}