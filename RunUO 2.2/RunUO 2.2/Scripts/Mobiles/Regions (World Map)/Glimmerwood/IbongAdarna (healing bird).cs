...
{
	[CorpseName( "an ibong adarna corpse" )]
	public class IbongAdarna : BaseCreature
	{
		[Constructable]
		public IbongAdarna() : base( AIType.AI_Animal, FightMode.Aggressor, 10, 1, 0.175, 0.350 )
		{
			Name = "an ibong adarna";
			Body = 832;
			BaseSoundID = 0x8F;
			Hue = 0x851;
		...
		{      
 			if ( !Flying && Utility.RandomDouble() < 0.25 )
				 ...
				 this.FixedParticles( 0, 10, 0, 0x2522, EffectLayer.Waist );
                                 Flying = true;
				 from.SendMessage( "Ibong Adarna activates its energy shield" );
                        }
		...
		public override void OnDamagedBySpell( Mobile from )
		{
			if ( Flying )
			{
				this.FixedParticles( 0x3735, 1, 30, 0x251F, EffectLayer.Waist );
                                Flying = false;
				from.SendMessage( "Ibong Adarna's energy shield has been broken" );
			}
		}
		...
		public override void OnGotMeleeAttack( Mobile attacker )
		{
			...
			if( attacker != null && attacker.Alive && attacker.Weapon is BaseRanged )
			{
			        ...
			        {
				     this.FixedParticles( 0x3735, 1, 30, 0x251F, EffectLayer.Waist );
				     attacker.SendMessage( "Ibong Adarna's energy shield has been broken" );
                                     Flying = false;
			        }
			}
			else if ( Flying )
                ...
                public override void OnCarve( Mobile from, Corpse corpse, Item with )
		{
			if ( corpse.Carved == false )
			{
			      base.OnCarve( from, corpse, with );
                              ...
                              corpse.AddCarvedItem( new Gold( Utility.RandomMinMax( 110, 160 ) ), from );
		...
		public void OnTick()
		{
	        ...
	        public class AOEHealTimer : Timer 
	        {
		        ...
		        public static List<IbongAdarna> AOEHealList = new List<IbongAdarna>();
		        ...
		        {
			        foreach (IbongAdarna ibongadarna in AOEHealList)
				        ibongadarna.OnTick();
		        }
	        }
		...
		public IbongAdarna( Serial serial ) : base( serial )
		{
		}
