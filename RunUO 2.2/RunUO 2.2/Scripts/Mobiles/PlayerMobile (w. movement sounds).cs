...
namespace Server.Mobiles
{
	...
	public enum PlayerFlag // First 16 bits are reserved for default-distro use, start custom flags at 0x00010000
	{
		...
		Infravision			= 0x00004000,
		HasFinished30Quests		= 0x00008000,
		HasFinished50Quests		= 0x00010000,
		HasFinished70Quests		= 0x00020000,
		HasMetGateKeeper		= 0x00040000,
		AreaAccessDesertRegion		= 0x00080000
	}
	...
	public class PlayerMobile : Mobile, IHonorTarget
	{
		...
		private int m_TotalQuestsDone;
		private int m_RacialNumber;
		...
		*/
		...
		[CommandProperty( AccessLevel.GameMaster )]
		public int TotalQuestsDone
		{
			get{ return m_TotalQuestsDone; }
			set{ m_TotalQuestsDone = value; }
		}

		[CommandProperty( AccessLevel.GameMaster )]
		public int RacialNumber
		{
			get{ return m_RacialNumber; }
			set{ m_RacialNumber = value; }
		}
		...
		{
			get{ return GetFlag( PlayerFlag.Infravision ); }
			set{ SetFlag( PlayerFlag.Infravision, value ); }
		}

		[CommandProperty( AccessLevel.GameMaster )]
		public bool HasFinished30Quests
		{
			get{ return GetFlag( PlayerFlag.HasFinished30Quests ); }
			set{ SetFlag( PlayerFlag.HasFinished30Quests, value ); }
		}

		[CommandProperty( AccessLevel.GameMaster )]
		public bool HasFinished50Quests
		{
			get{ return GetFlag( PlayerFlag.HasFinished50Quests ); }
			set{ SetFlag( PlayerFlag.HasFinished50Quests, value ); }
		}

		[CommandProperty( AccessLevel.GameMaster )]
		public bool HasFinished70Quests
		{
			get{ return GetFlag( PlayerFlag.HasFinished70Quests ); }
			set{ SetFlag( PlayerFlag.HasFinished70Quests, value ); }
		}

		[CommandProperty( AccessLevel.GameMaster )]
		public bool HasMetGateKeeper
		{
			get{ return GetFlag( PlayerFlag.HasMetGateKeeper ); }
			set{ SetFlag( PlayerFlag.HasMetGateKeeper, value ); }
		}

		[CommandProperty( AccessLevel.GameMaster )]
		public bool AreaAccessDesertRegion
		{
			get{ return GetFlag( PlayerFlag.AreaAccessDesertRegion ); }
			set{ SetFlag( PlayerFlag.AreaAccessDesertRegion, value ); }
		}
		#endregion
		...
		public override void ComputeBaseLightLevels( out int global, out int personal )
		{
			...
			if ( this.LightLevel < 8 && ( AosAttributes.GetValue( this, AosAttribute.NightSight ) > 0 || racialNightSight ))
				personal = 8;
			else
		...
		public override int StamMax
		{
			get
			{
				...
				if ( Core.AOS )
				{
					dexBase = this.Dex;	//this.Dex already includes GetStatOffset/dex
		...
		public override int ManaMax
		{
			get
			{
				...
				if ( Core.AOS )
				{
					intBase = this.Int;	//this.Int already includes GetStatOffset/int
...
////////////////////// find out if the player is moving over a land/terrain (ground), static/frozen (dungeon) swamp, or static/unfrozen/added swamp.
		...
		private static bool IsDeepWaterSurface( int itemID )
		{
			...
			if ( itemID >= 22000 && itemID <= 22001 ) // playsound 0x133, 0x134
				return true;
...
////////////////////////////////////// end poisonous swamps
			...
			if ( this.Alive && !wasAlive )
			{
				...
				if ( !EquipItem( deathRobe ) )
					deathRobe.Delete();
                                ...
                                else if ( this.Title == "the Ophidian" )
                                {
                                ...
                                else if ( this.Title == "the Anasiroch" )
                                {
                                        if ( this.Female == false )
                                        {
                                              this.BodyMod = 390;
                                ...
                                else if ( this.Title == "the Valborix" )
                                {
                                        if ( this.Female == false )
                                        {
                                              this.BodyMod = 123;
				...
				else if ( this.Title == "the Running Pants" )
				{
                                 	this.BodyMod = 430;
                                 	this.HueMod = 0;
                        	}
		...
		public override double RacialSkillBonus
		{
			get
			{
                ...
                public override void OnWarmodeChanged()
                {
                        if ( !Warmode )
                        {
                                Timer.DelayCall( TimeSpan.FromSeconds( 7 ), new TimerCallback( DungeonMusic ) );
                        }
                        else if ( Warmode )
                        {
                                this.Send( Network.PlayMusic.GetInstance( MusicName.Invalid ) );
                ...
                {
                        this.Send( Network.PlayMusic.GetInstance( this.Region.Music ) );
                }
		...
		private DateTime m_NextCookingBulkOrder;
		...
		public TimeSpan NextFletchingBulkOrder
		{
			get
			{
				TimeSpan ts = m_NextFletchingBulkOrder - DateTime.Now;
			...
			set
			{
				try{ m_NextFletchingBulkOrder = DateTime.Now + value; }
				catch{}
			}
		}
		...
		public TimeSpan NextCookingBulkOrder
		{
			get
			{
				TimeSpan ts = m_NextCookingBulkOrder - DateTime.Now;
			...
			set
			{
				try{ m_NextCookingBulkOrder = DateTime.Now + value; }
				catch{}
			}
		}
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 29:
				{
					   ...
					   NextFletchingBulkOrder = reader.ReadTimeSpan();
					   NextCarpentryBulkOrder = reader.ReadTimeSpan();
					   NextCookingBulkOrder = reader.ReadTimeSpan();
                                        }*/
					goto case 28;
				}
				case 28:
				...
				case 16:
				{
					...
					m_TotalQuestsDone = reader.ReadEncodedInt();
					m_RacialNumber = reader.ReadEncodedInt();
					goto case 15;
				}
				case 15:
				{
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			//writer.Write( NextFletchingBulkOrder );
			//writer.Write( NextCarpentryBulkOrder );
			//writer.Write( NextCookingBulkOrder );
			...
			writer.WriteEncodedInt( (int) m_TotalQuestsDone );

			writer.WriteEncodedInt( (int) m_RacialNumber );
